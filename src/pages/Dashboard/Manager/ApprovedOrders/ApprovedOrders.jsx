...
export default function ApprovedOrdersPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(`/orders/approved?email=${user.email}`);
      return res.data;
    },
  });
  ...
  return (
    <>
      <p>this is all approved orders page: {orders.length}</p>
      {orders.length > 0 ? (
                        ...
                        <dialog ref={updateStatusModalRef} className="modal">
                          <div className="modal-box">
                            ...
                            {/* {selectedOrder?.deliveryStatus === "ready" && (
                              <p className="border border-secondary/30 bg-base-300/50 text-primary font-semibold px-6 py-4 rounded-md text-center mt-3">
                                Ready to Deliver
                              </p>
                            )} */}
